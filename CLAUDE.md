...
6. **Lock before write.** Hooks must use the current runtime lock protocol before file/DB writes. SessionStart and Stop currently share the atomic lock directory `~/.claude/memory-system/.memory.lock`; update `PROJECT_MAP.md` and hook docs if this changes.
